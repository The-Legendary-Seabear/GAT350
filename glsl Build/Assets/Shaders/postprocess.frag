...
void main()
{
	...
	if ((u_parameters & GRAYSCALE) != 0u)      postprocess = vec4(average, average, average, 1);
if ((u_parameters & COLORTINT) != 0u)     postprocess *= u_colorTint;
if ((u_parameters & SCANLINE) != 0u)      postprocess = (int(gl_FragCoord.y) % 3 != 0) ? vec4(0,0,0,1) : postprocess;
if ((u_parameters & GRAIN) != 0u)         postprocess *= random(gl_FragCoord.xy + u_time);
if ((u_parameters & INVERT) != 0u)        postprocess = vec4(1.0 - color.rgb, postprocess.a);
